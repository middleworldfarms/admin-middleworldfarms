...
<div class="succession-planner-container">
    ...
    <script>console.log('🔄 Cache buster: {{ time() }} - RED LINE FIX');</script>
                    ...
                    <div class="harvest-window">
                        ...
                        <div class="mt-4">
                            ...
                            <div id="harvestTimeline" class="position-relative bg-light p-3" style="height: 100px; overflow: hidden; border-radius: 10px;">
                                ...
                                <div id="currentDateMarker" class="position-absolute" style="top: 0; bottom: 0; width: 4px; background: #ff0000; z-index: 20; box-shadow: 0 0 10px rgba(255, 0, 0, 0.8); border-left: 2px solid #990000; left: 20% !important;">
                                    <!-- Date label -->
...
<script>
    ...
    function updateCurrentDateMarker() {
        // RED LINE IS ALWAYS FIXED AT 20% - NEVER MOVES
        // The months/timeline moves instead to align with the fixed red line
        const FIXED_RED_LINE_POSITION = 20; // Fixed at 20% from left
        const marker = document.getElementById('currentDateMarker');
        
        if (marker) {
            // Force the position with !important override in JavaScript
            marker.style.setProperty('left', FIXED_RED_LINE_POSITION + '%', 'important');
            marker.style.display = 'block';
            console.log(`📅 FORCED red line to stay at ${FIXED_RED_LINE_POSITION}% - should not move!`);
            
            // Now update the timeline months to align with the fixed red line
            updateTimelineToAlignWithFixedRedLine();
        } else {
            console.error('❌ Could not find currentDateMarker element');
        }
    }

    function updateTimelineToAlignWithFixedRedLine() {
        // TODO: Update month labels and timeline scale so current date aligns with fixed 20% red line
        // The timeline months should slide to align with the fixed red line position
        console.log('🗓️ Timeline months should adjust to align with fixed red line at 20%');
        
        // This will be the function that moves the month labels/dates, not the red line
        // For now, just prevent any other functions from moving the red line
        
        // Make sure no other function tries to move the red line
        const marker = document.getElementById('currentDateMarker');
        if (marker && marker.style.left !== '20%') {
            console.log('⚠️ CORRECTING red line position - was moved by another function');
            marker.style.setProperty('left', '20%', 'important');
        }
    }
