...
app = FastAPI(
    ...
    description="Laravel bridge to Phi-3 Mini for succession planning with RAG",
    version="1.0.0"
)
...
async def root():
    return {
        ...
        "status": "operational",
        "rag_enabled": is_rag_enabled() if RAG_AVAILABLE else False,
        "features": ["succession_planning", "biodynamic_farming", "vector_search"] if RAG_AVAILABLE and is_rag_enabled() else ["succession_planning", "biodynamic_farming"]
    }

@app.post("/ask")
async def ask_phi3(request: AskRequest):
    """Main endpoint - connects Laravel to Phi-3 Mini with RAG support"""
    try:
        ...
        if ollama_response.status_code == 200:
            ...
            return {
                ...
                "confidence": "high",
                "rag_used": RAG_AVAILABLE and is_rag_enabled()
            }
        else:
...
"""
    ...
    # Use RAG to enhance prompt if available
...
if __name__ == "__main__":
    ...
    print(f"🔍 RAG Support: {'Enabled' if RAG_AVAILABLE and is_rag_enabled() else 'Disabled'}")
